...
/**
 * A super moderator, so the moderator's screens have somebody to be.
 *
 * Deliberately **not** an administrator. `admin` bypasses forum permissions
 * entirely (R4.2), so a moderation spec driven as `admin` proves the bypass
 * works and says nothing about whether the moderator path does — and the
 * moderator path is the one nearly every board actually runs on. Super
 * moderator rather than plain moderator because `forum_moderators` assignment
 * is a separate feature with its own rows; scoping it per forum belongs in a
 * spec about scoping, not in every spec that needs a queue.
 *
 * The id and the name live in `config.ts` with the ports, because a spec has to
 * read them and cannot import this file — see that file's header.
 */
...
function seedSql(staffHash: string): string {
  ...
  const users = [...authors].map(([id, username]) => {
    ...
    return {
      ...
      /*
       * A hash nothing can match, **except for `admin`**.
       *
       * The ordinary members here exist to own content rather than to be
       * signed into — a spec that needs a member registers through the form,
       * which is the path worth exercising anyway.
       *
       * `admin` is the exception because there is no other way to be one. A
       * registration always lands in the Registered group, so a suite where
       * every account is registered can never open the control panel, act on
       * the moderation queue or close a report — which is exactly why those
       * three had no browser coverage at all while ten features were built on
       * them. Staff is not a thing the board hands out; it has to be seeded.
       */
  ...
  /**
   * The moderator, with a password that works.
   *
   * Its own block rather than a row in `users` above, because that list is
   * derived from who wrote the seeded posts and this account has written none —
   * it exists to *act on* what other people wrote.
   *
   * Group 4 (Super Moderators) is the whole point: it carries `can_access_mod_cp`
   * and `is_super_moderator` and **not** `can_access_admin_cp`, so a spec driven
   * as this member proves the moderator's own path rather than the
   * administrator bypass — and proves, in the same breath, that the ACP is shut
   * to somebody who moderates every forum on the board.
   */
  const staff = [
    insert('users', [
      {
...
export async function startDatabase(
  ...
  if (seeded) {
    ...
    /*
     * Hashed here rather than checked in as a literal. The parameters travel
     * with the hash, so a checked-in string would go on verifying against
     * whatever `hashPassword` was configured with on the day it was pasted —
     * and a board that had since raised its cost would accept it anyway, which
     * is a fixture quietly disagreeing with the code it is meant to exercise.
     * It costs about eighty milliseconds, once, at start-up.
     */
