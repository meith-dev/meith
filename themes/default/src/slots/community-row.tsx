...
/**
 * One community in a listing (F29).
 *
 ...
 * readers, because "this community has new posts" is information, and information
 ...
 * it the listing indents and un-indents as communities are read, which is the sort of
...
export function CommunityRow({ community }: CommunityRowSlotModel) {
  ...
  return (
    <li
      data-unread={community.isUnread ? '' : undefined}
      className={
    ...
    >
      {community.isUnread ? <UnreadDot /> : <ReadSpacer />}

      <div className="min-w-0">
        <a
          href={community.href}
          className={
            (community.isUnread ? 'font-semibold text-foreground' : 'font-medium text-foreground') +
            ` ${LINK}`
          }
        >
          {community.title}
        ...
        {community.subcommunities.length > 0 && (
          <p className="mt-1.5 flex flex-wrap items-baseline gap-x-2 gap-y-0.5 text-xs text-muted-foreground">
            <span className="font-medium text-foreground">Subcommunities</span>
      ...
      {!isLink && (
        <>
           ...
           * and the rows stop lining up — which on a list of twenty communities reads
          ...
          <div className="col-start-2 min-w-0 text-xs text-muted-foreground md:col-start-4">
            {community.lastPost === null ? (
              <span className="text-community-read">No posts yet</span>
            ) : (
              <>
                <a
                  href={community.lastPost.href}
                ...
                <span className="mt-0.5 block truncate">
                  {'by '}
                  <UserRef user={community.lastPost.author} className="font-normal" />
                  {' · '}
                  <Stamp at={community.lastPost.at} />
                </span>
              </>
            )}
          </div>
