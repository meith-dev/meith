...
export function LatestThreads({
  ...
  const c = (key: string) => fromSlotCopy(copy, `meith.latestThreads.${key}`)

  return (
    <section aria-labelledby="latest-threads-heading" className={`${EDITORIAL_RULE} pt-4`}>
      ...
      {threads.length === 0 ? (
        ...
        <ul className="mt-3">
          {threads.map((thread) => (
            <li key={thread.href} className={`${RULE} flex flex-col gap-0.5 py-3`}>
