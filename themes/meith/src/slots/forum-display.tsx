...
export function ForumDisplay({
  ...
  const c = (key: string) => fromSlotCopy(copy, `meith.forumDisplay.${key}`)
  ...
  return (
    <div className={PAGE_BODY}>
      ...
      {regions.tools !== undefined && (
        <div className="-mb-4 flex flex-col gap-3 empty:hidden">{regions.tools}</div>
      )}

      <section aria-label={c('threadsLabel')} className={`${EDITORIAL_RULE} @container/card pt-4`}>
        <div
          className={`${LABEL} hidden grid-cols-[minmax(0,1fr)_9rem_15rem] gap-x-6 pb-3 @3xl/card:grid`}
        >
        ...
        {isEmptyRegion(regions.threads) ? (
          <div className={`${RULE} flex flex-col items-start gap-3 py-10`}>
            <p className="text-[1.02rem] font-medium text-foreground">{c('noThreadsYet')}</p>
