...
export function Announcement({
  ...
  const c = (key: string) => fromSlotCopy(copy, `meith.announcement.${key}`)

  return (
    <article className={`${EDITORIAL_RULE} pt-4 pb-6`}>
      <Label>{c('label')}</Label>
      <h2 className={`${SECTION_TITLE} mt-3 text-balance`}>{title}</h2>

      <div
        className={`prose-md mt-4 ${BODY_MEASURE} text-[0.9375rem] leading-[1.75]`}
