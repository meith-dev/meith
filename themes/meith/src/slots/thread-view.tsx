...
export function ThreadView({
  ...
  const c = (key: string) => fromSlotCopy(copy, `meith.threadView.${key}`)
  ...
  return (
    <div className={PAGE_BODY}>
        ...
        <div className="flex flex-wrap items-center gap-x-6 gap-y-3 lg:justify-end">
          ...
          {watch != null && (
            <form action={watch.action} method="post">
              <button
                type="submit"
                className={`${LABEL} inline-flex items-center gap-1 py-2 ${QUIET_LINK} ${TOUCH}`}
              >
                {watch.subscribed ? (
                  <>
      ...
      <div className="flex flex-col gap-8">{regions.posts}</div>
