...
import { Arrow, LABEL, Mark, NUMERIC, QUIET_LINK, Stamp, UserRef } from '../shared'
...
export function WhoIsOnline({
  ...
  const c = (key: string) => fromSlotCopy(copy, `meith.whoIsOnline.${key}`)
  ...
  return (
    ...
    >
      ...
      <span className="flex shrink-0 flex-wrap items-baseline gap-x-4 gap-y-1">
        <a href={fullListHref} className={`group font-medium text-foreground ${QUIET_LINK}`}>
          {c('seeEveryone')}
          <Arrow />
        </a>
        {recordAt !== null && (
  ...
  return (
    <>
      ...
      {member.isInvisible && (
        <>
          {' '}
          <Mark>{fromSlotCopy(copy, 'meith.whoIsOnline.invisible')}</Mark>
        </>
      )}
    </>
  )
