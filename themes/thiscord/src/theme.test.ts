...
import { thiscordTheme } from './theme'
...
describe('the thiscord theme', () => {
  it('satisfies the theme-kit contract', () => {
    expect(assertThemeContract(resolveTheme(thiscordTheme)).missing).toEqual([])
  })

  it('inherits from the default theme rather than copying it', () => {
    expect(resolveTheme(thiscordTheme).chain).toEqual(['thiscord', 'default'])
  })

  it('fills every stable slot itself, so no page falls back to another theme’s markup', () => {
    const own = new Set(Object.keys(thiscordTheme.slots))
  ...
  it('shares no slot implementation with the theme it extends', () => {
    const resolved = resolveTheme(thiscordTheme)

    const shared = Object.keys(thiscordTheme.slots).filter(
      (name) =>
