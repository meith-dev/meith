...
import { BTN, BTN_PRIMARY, Circle, MUTED_LINK, count } from '../shared'

const ICON =
  'inline-flex size-8 shrink-0 items-center justify-center rounded-sm text-muted-foreground transition-colors hover:bg-accent hover:text-foreground'

function BellIcon() {
  return (
    <svg aria-hidden="true" viewBox="0 0 20 20" className="size-5" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
...
function CountBadge({ value, label }: { value: number; label: string }) {
  return (
    <span className="absolute -top-1 -right-1 inline-flex min-w-4.5 items-center justify-center rounded-full bg-destructive px-1 text-[0.6875rem] font-bold text-destructive-foreground ring-2 ring-card">
...
function IconButton({
  ...
  if (href === null) {
    return (
      <span className={`relative ${ICON}`} aria-hidden="true">
        {children}
      </span>
    )
  }
  ...
  return (
    <a href={href} title={title} className={`relative ${ICON}`}>
    ...
    return (
      <div className="flex items-center gap-2">
        {links.map((link, index) => (
          <a key={link.href} href={link.href} className={index === 0 ? BTN_PRIMARY : BTN}>
            {link.label}
          </a>
        ))}
      </div>
