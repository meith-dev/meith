    ...
    >
      {posts.length === 0 ? (
        ...
        <ul className="px-1 pt-1 pb-1">
          {posts.map((post) => (
            <li key={post.href} className="rounded-sm px-2 py-1.5 transition-colors hover:bg-accent">
              <div className="flex items-start gap-2.5">
                <Circle name={post.author.username} size={28} className="mt-0.5" />
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 truncate text-xs text-muted-foreground">
                    <UserRef user={post.author} className="text-xs" />
                    {' in '}
