...
export function ForumRow({ forum, copy }: ForumRowSlotModel & { copy: SlotCopy }) {
  ...
  const c = (key: string) => fromSlotCopy(copy, `vershell.forumRow.${key}`)

  return (
    <li
      data-unread={forum.isUnread ? '' : undefined}
      className={`group relative grid gap-x-6 gap-y-2 px-2 py-4 first:border-t-0 ${ITEM_RULE} ${ROW_HOVER} ${
