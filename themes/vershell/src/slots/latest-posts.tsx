...
export function LatestPosts({ posts, capturedAt, copy }: LatestPostsModel & { copy: SlotCopy }) {
  const c = (key: string) => fromSlotCopy(copy, `vershell.latestPosts.${key}`)

  return (
    <section aria-labelledby="latest-posts-heading" className={`${CARD} px-5 pt-5 pb-2`}>
      ...
      {posts.length === 0 ? (
        ...
        <ul className="mt-2">
          {posts.map((post) => (
            <li key={post.href} className={`${RULE} flex flex-col gap-0.5 py-3 first:border-t-0`}>
              <a
                href={post.href}
