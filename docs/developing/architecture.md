...
read [Development](./development.md); for framework
conventions, [Next.js conventions](./nextjs-conventions.md).
...
[Consuming the board from a workspace](./development.md#consuming-the-board-from-a-workspace).
...
`meith.demo.plugins.ts` each pulls in ([Plugins](./plugins.md)
...
[Monitoring](../operating/monitoring.md#driving-the-tick-over-http).
...
[Scaling out](../operating/scaling.md) for the supported setup.
...
[Theme slots and view models](../reference/theme-slots.md), and slot checks verify
...
[Plugin hooks](../reference/plugin-hooks.md) is generated from the current registry.

## REST API

API v1 routes are declared in a registry that generates
[REST API v1](../reference/api.md) and `docs/reference/openapi.json`. Route handlers
