...
in [Installing plugins and themes](../getting-started/installing.md). The rest of this section
...
[Languages](../operating/internationalisation.md) covers the message syntax, the plural
...
[Maximum displayed groups](../using/groups.md#display-groups)
...
and a [pushed notification](../operating/operating.md#web-push) if the board offers push and the
...
- **`schedule`** is a five-field cron expression — `minute hour
  ...
  reader's](../getting-started/mybb-parity.md)), so UTC is the only honest anchor,
