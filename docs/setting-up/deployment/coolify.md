...
[the marketplace](../../developing/marketplace.md): a board built this way is yours from
...
want to read the code or write a theme, [Development](../../developing/development.md)
...
a workspace](../../developing/development.md#consuming-the-board-from-a-workspace)
...
2. Either pick the bucket as the **Off-site destination** under
   ...
   **Redeploy**. All four required values or none; the environment wins
   ...
   is what [Disaster recovery](../../operating/disaster-recovery.md#under-coolify)
   leans on.
...
[Backups](../../operating/backups.md) is the full reference —
...
2. **Terminal → `web`** — for a full restore including uploads, empty the
   ...
   [Downgrades](../../operating/upgrading.md#downgrades) before
   deciding which.
...
[Disaster recovery](../../operating/disaster-recovery.md), which
has the order of operations and the verification list; its
[Under Coolify](../../operating/disaster-recovery.md#under-coolify)
...
- **It failed after the administrator was created.** The installer
  ...
  [operator CLI](../../operating/operating.md#account-recovery) —
  `meith user:promote`.
