...
| F04 | Deploy on both targets | `PARTIAL` | Dockerfile (multi-stage, standalone) + `docker-compose.yml`; CI builds the app. **`apps/worker` is now real**: an in-process loop calling `tick()` on the same task code the app runs, refusing fixture mode, finishing the tick in flight on SIGTERM. One image, three roles behind `FORUM_ROLE` (`web`/`worker`/`migrate`) via an entrypoint, so they cannot drift. A new CI `image` job **builds the image and boots it**, and building it for the first time found five things that had rotted unseen, all fixed: no `.dockerignore` at all (so a developer's gitignored `apps/forum/.env` was copied in, and its `QUEUE_DRIVER=postgres` broke the fixture build — patterns must be `**/.env`, since dockerignore is anchored to the context root); the build stage copied two of the many pnpm workspace `node_modules` trees and only worked because the host's were being dragged in behind it; three package manifests were missing from a hand-maintained COPY list, now a `--parents` glob; the image took *whatever pnpm was latest* (11.18) while dev used 10.6 and CI pinned 9, now one `packageManager`; and compose's migrate service ran `drizzle-kit`, which is not in the pruned standalone tree. **Gap:** the **web role is not booted** by the job. Doing so surfaced a *pre-existing, intermittent* failure — `TypeError: c is not a function` from `container.ts`'s lazy `require('@forum/db')` under Turbopack — which reproduces on `main` without any of this work and makes `pnpm build` fail about two runs in three **when `DATABASE_URL` is set**. CI has never seen it because its `build` job runs `DATA_SOURCE=fixture`, which never executes that require: the Postgres build path has never been built or booted anywhere. That is its own change. |
