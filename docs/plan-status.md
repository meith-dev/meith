...
Last audited **2026-07-30** (re-audited after F40 landed), against the working tree, not from memory:
`pnpm verify` (1058 tests), `pnpm build`, `pnpm test:e2e`, plus direct inspection of the
...
registered member can start a thread and reply to one. `packages/bbcode` still
does not exist, so post bodies are stored raw and rendered as escaped plain
text — including the BBCode a quote prefills.
