...
**91 hooks** — 46 filters, 45 events — and 6 UI regions.
...
| Hook | Kind | Value | Context | Feature |
|---|---|---|---|---|
...
| Hook | Kind | Value | Context | Feature |
|---|---|---|---|---|
| `view.header` | filter | `HeaderModel` | `ViewerRef & RequestRef` | F27 |
| `view.user-panel` | filter | `UserPanelModel` | `ViewerRef & RequestRef` | F27 |
| `view.navigation` | filter | `NavigationModel` | `ViewerRef & RequestRef` | F27 |
| `view.footer` | filter | `FooterModel` | `ViewerRef & RequestRef` | F27 |
| `view.board-index` | filter | `BoardIndexModel` | `ViewerRef` | F29 |
| `view.forum-row` | filter | `ForumRowSlotModel` | `ViewerRef` | F29 |
| `view.thread-row` | filter | `ThreadRowSlotModel` | `ViewerRef & ForumRef` | F30 |
| `view.post-bit` | filter | `PostBitSlotModel` | `ViewerRef & ThreadRef` | F31 |
| `view.post-actions` | filter | `PostActionsSlotModel` | `ViewerRef & ThreadRef` | F31 |
| `view.member-profile` | filter | `MemberProfileModel` | `ViewerRef` | F33 |
| `view.board-stats` | filter | `BoardStatsModel` | `ViewerRef` | F75 |
| `view.who-is-online` | filter | `WhoIsOnlineModel` | `ViewerRef` | F75 |
| `view.pagination` | filter | `PaginationModel` | `ViewerRef` | F30 |
| `view.search-form` | filter | `SearchFormModel` | `ViewerRef` | F73 |
| `view.error-notice` | filter | `ErrorNoticeModel` | `ViewerRef & RequestRef` | F34 |
| `view.shell` | filter | `ShellModel` | `ViewerRef & RequestRef` | F27 |
| `view.notice` | filter | `NoticeModel` | `ViewerRef` | F27 |
| `view.category-block` | filter | `CategoryBlockModel` | `ViewerRef` | F29 |
| `view.subforum-list` | filter | `SubforumListModel` | `ViewerRef & ForumRef` | F30 |
| `view.forum-display` | filter | `ForumDisplayModel` | `ViewerRef & ForumRef` | F30 |
| `view.thread-view` | filter | `ThreadViewModel` | `ViewerRef & ThreadRef` | F31 |
...
| `thread.created` | event | `ThreadRef & { authorId: number; subject: string }` | `ViewerRef` | F39 |
...
| `post.created` | event | `PostRef & { authorId: number }` | `ViewerRef` | F40 |
| `post.edit.before` | filter | `{ readonly body: string; readonly reason: string \| null }` | `PostRef & ViewerRef` | F41 |
| `post.edited` | event | `PostRef & { editorId: number; revision: number }` | `ViewerRef` | F41 |
