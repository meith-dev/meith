...
This guide is for wiring a board into whatever your organisation already uses to watch its infrastructure: what to scrape, what to alert on, and how the pieces relate to each other. It assumes the [Compose deployment](../setting-up/deployment/docker-compose.md) — `web`, `worker`, `postgres`, and `migrate` running to completion.
...
| Rich detail on tasks, search index, recounts, mail | The admin panel's System Health page — see the [Organiser guide](../using/organiser-guide.md) |
