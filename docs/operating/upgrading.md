...
[release policy](../developing/release.md#the-version-policy) is that a patch never
...
([Releasing § Deploy template repositories](../developing/release.md#deploy-template-repositories)):
...
([Building where Vercel looks](../developing/development.md#building-where-vercel-looks)) —
...
installing. See
[The organiser's guide § Registration](../using/organiser-guide.md#registration).
...
[Spam controls and rate limits § The three login counters](../using/antispam.md#the-three-login-counters).
...
mean both. See
[Forums and permissions § What an appointment grants](../using/forums.md#what-an-appointment-grants)
...
[letting members delete their own threads](../using/forums.md#letting-members-delete-their-own-threads)
...
[the daily post allowance](../using/groups.md#the-daily-post-allowance).
...
[the post anchor](../developing/themes.md#the-post-anchor) is the shape.
...
off. See
[the limits on pages nobody has signed in to](../using/antispam.md#the-limits-on-pages-nobody-has-signed-in-to).
...
[the theme API](../developing/themes.md#versioning).
