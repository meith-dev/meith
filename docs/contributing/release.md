...
| `ghcr.io/meith-dev/meith-base:X.Y.Z` | The framework base image a scaffolded custom board's own Dockerfile starts `FROM` — the `@meith/web`/`@meith/cli`/`@meith/theme-default` dependency closure, version-locked to this release, no board config and no secret (`docker/Dockerfile.base`). No floating tag: a board pins it exactly, the same way it pins the npm packages below. See [Self-hosting § Custom boards](../getting-started/deployment/docker-compose.md#custom-boards). |
...
- The `version` field of every listing in `marketplace/listings/` whose
  ...
  see [the marketplace](../customization/marketplace.md#what-is-in-a-listing). A
...
checked: `docs/reference/openapi.json` stamps the release version as its
`info.version`, and `apps/web/public/create-board.sh` — the installer
[the Quickstart](../getting-started/deployment/coolify.md#2-create-your-board) points operators at
...
`docs/reference/openapi.json`. `pnpm release:bump` moves the six listings and
...
[the marketplace § What is seeded today](../customization/marketplace.md#what-is-seeded-today).
...
2. **Run the "Cut a release" workflow** — Actions → *Cut a release* → the
   ...
   version, `docs/reference/openapi.json` and the marketplace feed among them, proves
...
(`@meith/worker`) has no [board-config seam](../reference/architecture.md#the-board-config-seam)
...
[plugin API versioning](../customization/plugins.md#versioning)): a minor may add
...
- **`boards/stock`** — the workspace `docker/Dockerfile` builds the official
  image from (see docs/reference/architecture.md, "The stock board"). It is a board,
