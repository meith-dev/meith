# Release infrastructure and version policy

Understand what a release publishes and how packages, images and templates stay in step. Use the release procedure for the ordered maintainer actions.
...
| `marketplace/listings/*.json` whose `package` names a workspace package | The first-party listings — see [the marketplace](../extensions/marketplace.md) | `release:check`, over **every** listing in the directory |
| `docs/reference/openapi.json` | `info.version` of the generated API reference | `api:docs:check` |
| `apps/web/public/create-board.sh` | The installer [the Quickstart](../operations/coolify.md) points operators at, so nobody needs Node.js on their own machine | `board-installer:gen:check` |
...
- **`@meith/worker`** (`apps/worker`) — no [board-config seam](architecture.md)
...
([theme API versioning](../extensions/theme-contract.md#versioning),
[plugin API versioning](../extensions/plugin-hooks-guide.md#versioning)): a minor may
...
of truth, regenerated by `pnpm release:bump` through `pnpm templates:gen`
from `create-meith`'s `scaffold()`. Between releases the committed trees stay
unchanged even when the scaffold or its dependencies change. The release
workflow runs `pnpm templates:gen:check` before publishing npm packages;
normal CI tests the generator and the released template structure without
requiring those trees to match an unreleased scaffold.
...
keep ([Upgrading § Upgrading each deployment route](../operations/upgrading.md)).
...
|---|---|---|
| [Quickstart](../operations/coolify.md) (Coolify) | The `release` branch; the compose pin on the exact version, held per resource by `MEITH_IMAGE` | Every release moves the pin, so an upgrade is a **Redeploy** after the branch moves — or automatic, with the webhook on. Never from a push to `main`. Coolify's **Restart** re-runs the deployment from the branch head, so unpinned it upgrades too — the Quickstart has the operator pin `MEITH_IMAGE` so it cannot. |
| A scaffolded, template or [by-hand](../operations/docker-compose.md) board ([Upgrading](../operations/upgrading.md)) | Its own `package.json`, pinned exact | Its `.github/workflows/update.yml` opens a pull request on its weekly run — or `npx create-meith@latest update` by hand, then `docker compose up -d --build` (or a Coolify Redeploy) to actually deploy it. Either moves the pins and the scaffold-owned deploy files together; the backup and `meith upgrade` stay the operator's. |
| meith.dev | `main` | The project website redeploys on push, ahead of a release. |
| [forum.meith.dev](https://forum.meith.dev) | Its own template repository, with exact package pins | The live community follows the template-board upgrade route. |
