...
- **`community.config.ts`** is the build-time registry. It calls
...
- **`board.plugins.json`** is the plugin manifest — the list a human
  edits. Adding a plugin is `npm install`, an entry here, and a redeploy.

- **`community.plugins.ts`** turns that manifest into typed imports for
  the config to consume. Inside the Meith repository it is generated by
  `pnpm board:gen` and checked by CI; a scaffolded board starts with a
  plain, valid file of the same shape and a comment showing the one-line
  entry a plugin adds.

Registering a theme is the same motion: install its package, add an
entry to the `themes` map in `community.config.ts`, set `defaultTheme`
