...
Import members and content from MyBB or phpBB, then verify the board before
cutover. The importer is resumable and reads from the source without changing
it. Rehearse against a copy first.

Review [MyBB differences](../reference/mybb-parity.md) or
[phpBB differences](../reference/phpbb-parity.md) and the coverage table below
...
| You need | Because |
|---|---|
| A Meith board, already installed | The importer writes into `users`, `forums` and the rest of the live schema — run [Coolify](../getting-started/deployment/coolify.md) or [Deploying by hand](../getting-started/deployment/docker-compose.md) first, including the `/install` step, so the default groups and settings the importer relies on already exist. |
...
before phpBB support existed). An inline environment assignment can still be saved in shell history.
Supply the variable through your deployment’s secret mechanism or a private
local environment file, and do not commit credentials.
...
[The markup language is Markdown, not BBCode](../reference/mybb-parity.md#the-markup-language-is-markdown-not-bbcode);
...
[phpBB parity decisions](../reference/phpbb-parity.md). Read whichever applies before
...
[Operations](./operations/operating.md#troubleshooting).

## Next

| You want to | Read |
|---|---|
| Understand where this board deliberately differs from MyBB before you promise members a like-for-like move | [MyBB parity decisions](../reference/mybb-parity.md) |
| Same question, coming from phpBB | [phpBB parity decisions](../reference/phpbb-parity.md) |
| Rebuild groups and forum permissions | [The organiser's guide](./community/organiser-guide.md) |
| Hand the approval queue and reports to your moderators | [The moderator's guide](./community/moderation-guide.md) |
| Run backups, upgrades and routine maintenance from here on | [Operations](./operations/operating.md) |
