...
belongs in its own catalog. [The theme API](../../customization/themes.md) has the theme side.
...
[Running on Vercel](../../getting-started/deployment/vercel.md#when-a-derivation-cannot-resolve) for what each
refusal means.
...
`/api/v1/openapi.json` and the `docs/reference/api.md` generated from it. That
