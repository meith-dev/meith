...
Both are held to the same floor as every other secret this board reads, **32 characters**, and the board refuses to boot on a shorter one. Note that this is stricter than the 16 characters [Vercel's own cron documentation](https://vercel.com/docs/cron-jobs/manage-cron-jobs#securing-cron-jobs) suggests for `CRON_SECRET`: a value generated by following those instructions is rejected here, and the fix is a longer secret rather than a lower floor. Generate one with:
...
### Telling Vercel when to call it

A Vercel deployment declares its schedule in `vercel.json` at the root of the project it deploys:
...
Vercel calls that path with `Authorization: Bearer <CRON_SECRET>` on the schedule given, and the every-minute schedule above is what matches the worker's own loop.

**Hobby projects cannot have that schedule.** The plan allows a couple of cron jobs and runs each of them about once a day, at an hour Vercel chooses; only paid plans accept an arbitrary cron expression. A board driven by Hobby cron therefore ticks daily, which — by the latency note under [Cadence](#cadence) — makes "as it happens" notifications a daily digest in all but name. A board that wants a minute-by-minute tick on Hobby drives the endpoint from something else that can call a URL on a schedule: a GitHub Actions workflow, a systemd timer, an uptime pinger, using `TICK_SECRET`.
...
On Vercel that figure is checked when the project **builds**, not when the function runs, so a plan that does not allow it fails the deployment rather than clamping the request. With [Fluid Compute](https://vercel.com/docs/fluid-compute) — the default for new projects — Hobby allows up to 300 seconds and the declaration builds as written. With Fluid Compute switched off, Hobby caps a function at 60 seconds and this build fails. Turn Fluid Compute back on, or lower the constant in `apps/community/app/api/system/tick/route.ts` to what the plan allows; a lower ceiling costs nothing but the tail of an unusually long tick, which the next tick picks up anyway.

**A bounded task trades reach per run for a fixed cost per run, and that is a trade rather than a speed-up.** `promotions.apply` is the clearest case. It used to evaluate every member on the board every six hours, so its cost grew with the membership and a large board risked a run that outlived the function it ran in. It now evaluates at most 10,000 members an hour — 240,000 a day — resuming from a stored cursor, so one run costs the same whatever the board's size. Below roughly 240,000 members the new scheme comes round at least as often as the old one did. Above it, it is slower: a board of a million members went from a full pass every six hours to a full pass every four days or so. Bounding is still the right call, because a promotion arriving late is a delay while a run killed mid-scan is work that never lands — but it is worth knowing which side of that line a board sits on. The page size and the interval are constants in `packages/tasks/src/builtin.ts`; there is no setting for them.
...
| `meith_task_run_duration_seconds` | histogram | `task` | How long each scheduled task takes. Compare against `docs/reference/performance.md`'s offline budgets when a task starts running long. |
...
Nothing here replaces `docs/reference/performance.md`'s offline harness — tracing shows *where* time went for one request in production; the harness is still how you catch a regression before it ships.
...
| Rich detail on tasks, search index, recounts, mail | The admin panel's System Health page — see the [Organiser guide](../community/organiser-guide.md) |
