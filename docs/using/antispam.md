...
Most of it ships switched off. What ships on is what no human ever
notices: the hidden-field trap, a three-second minimum fill time, and
the four pre-authentication limits below.

> [!NOTE]
> **The counters live in the database**, so every instance of your board
> shares one allowance. A board running without Postgres (fixture mode,
> `pnpm dev`, the demo) has no counters and applies none of the rate
> limits on this page.
...
- **A challenge switched on with no usable question does nothing** rather
  than refusing everybody. The screen says so in red at the top.

## The limits on pages nobody has signed in to

Four limits sit on the pages a visitor reaches before they have an
account, and unlike the rest of this screen they **ship switched on**:
...
or refused on a limit, so it never confirms that an address has an
account.
...
conference): ten accounts an hour from one /24 is generous for a board
and low for a lecture hall.

## The three login counters

A failed sign-in is counted three times over, and the three answer
different attacks. Two are account controls in the **security** settings
group; the third is a volume control and lives with anti-spam:
...
All three are measured over `security.lockout_minutes` (default 15). All
four settings are marked **advanced**, so the settings screen hides them
until you ask for advanced settings.
...
The middle one locks the **real owner** out too, which is the price of it
working against a botnet. Keep it well above the per-address number. A
...
A script satisfies any interval you would be willing to set, so use
...
Each is a bucket per member, or per requesting /24 for a signed-out
visitor, over a fixed hour. `0` disables the limit, and all five ship at
`0`.
...
`antispam.upload_per_hour` is one bucket for both kinds of upload: files
attached to a post cost one unit each, so six files in one post cost
six; a new avatar costs one unit per attempt, spent before the image is
examined, so a rejected image costs one too. Submitting the form with no
file chosen, or removing an avatar, costs nothing.
...
- A **question challenge** switched on with no question configured does
  nothing rather than refusing everybody.
...
- **Registrations per hour from one address** at its default of 10 is
  low for a hall full of people on one connection.

If registrations are *created* but nobody can sign in afterwards, it is
the activation method waiting for mail the board cannot send. See
[Operations § Mail](../operating/operating.md#mail).

## No hosted captcha

A hosted captcha means every visitor's browser contacting a third party
before they can join your board, which is a decision about your members
rather than a setting. The provider seam (`CaptchaProvider` in
`packages/antispam`) is there if you want one. See
[the plugin API](../developing/plugins.md).

## Community flagging

Set **Community flag threshold** (`moderation.flag_threshold`) to a
number *N* and, the moment *N* **different members** have an open
[report](./moderation-guide.md#reports) against the same visible post,
the board holds that post for approval, in the same database transaction
that files the *N*th report. The hold is logged as `post.autohold` in the
moderator log. `0`, the default, switches it off.

- **It counts distinct members, not reports.** One member cannot report
  the same post twice while their first report is open, so a single
  member can never trip the threshold alone. Only reports filed *since
  the post was last held* count towards holding it again, so approving a
  flag-held post from the queue makes it stick until a fresh set of
  distinct members reports it.
- **It is deliberately blunt.** A report from a brand-new or ignored
  account still counts. Weighting reports by who filed them is not a
  setting here.
- **It acts on the content, never the author.** Holding a post changes
  ...
  moderation bypass are untouched. A held post is reviewed from the
  approval queue like any other held content, and the reports stay open
  until a moderator closes them.
- **A reported opening post holds its whole thread; a reported reply
  holds just the reply.** Post and thread counts are decremented for
  everything held and restored in full on approval, so a rejected thread
  leaves nothing behind.

## The word filter

The word filter rewrites words as a page is rendered, without editing
anybody's post. `/admin/content` holds it, under **Word filters**. It is
an administrator's control: a moderator's route to bad language is the
warning ladder or a hidden post.
...
- **The replacement**: what to put in its place. It may be empty, which
  removes the word.
- **Whole word**: on, the pattern only matches when it stands alone as a
  word; off, it matches anywhere inside a longer one.
...
> `.*` matches the two characters `.` and `*` and nothing else. To catch
> several spellings of a word, write several rules.
...
brings the original word back everywhere immediately. Two consequences:
...
The filter steps over HTML tags, so it never rewrites a link's address,
a class name or an attribute: a rule for `cat` cannot break a link to
...
- **The thread resource the REST API returns** under `/threads`. It is
  the record a client may write back, and a filtered title must never
  become the stored one. The search results and subscription lists the
  API serves are display text, and they are filtered.
...
> e-mail and a push payload as well as a line on the board.

### What it costs

Very little. The compiled rules are cached board-wide and rebuilt when
you change one. The real cost is judgement: a filter that rewrites a
...
- **A note**: optional, seen only on this screen. Why the filter exists,
  so whoever reads it in six months knows whether it still needs to.

The screen records who added each filter and when, and both adding and
removing one are written to the admin log.
...
of the three matched**: naming the field would tell somebody trying to
get in exactly what to change.
