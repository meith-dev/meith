...
[Operations](../operating/operating.md), and this page says when to hand
something over.
...
You are appointed to forums, one at a time, and each appointment carries
exactly the rights the organisers ticked. Approving content, editing,
deleting, restoring, locking, pinning, moving, merging and splitting are
separate grants. See
...
**My forums** (**`/modcp/forums`**) lists every forum you are appointed
to and exactly what you may do in each. If a right is not in that list,
the board will refuse the act.
...
A plugin can add sections of its own to the panel. They appear under
**`/modcp/plugins/<plugin>`**, beside the sections above.

> [!NOTE]
> Everything you do as a moderator, every approval, deletion, lock,
> move and warning, is written to the moderator log with your name on
> it. That is what lets two moderators share a forum without wondering
> who did what.
...
- **The author is new.** The board has a setting for holding a member's
  first posts: **Hold a new member's first posts**, under
  `/admin/settings?group=antispam`, holds their posts until their post
  count reaches the number set. It ships at 0, which is off, so it only
  fills your queue on a board whose administrator has turned it on. See
...
- **Rejecting** moves it to the deleted state, the same reversible
  state a moderator's delete produces. Somebody with the restore right
  can bring it back.

Two rules decide what the queue lists:
...
The queue shows you the text **as written**, without the board's word
filter applied. See [Keeping it civil](#keeping-it-civil).
...
required only for *something else*. What they write is shown to
moderators and never to the person reported. A member reporting the same
thing again while their report is open does not create a second report.
...
and the reports screen can be **filtered to one category**, which helps
when a spam run has filled the list. Who sees a report depends on what
it is about:
...
Close dismissed reports rather than leaving them open.
...
(`moderation.flag_threshold`, under `/admin/settings?group=antispam`) to
a number *N*, and once *N* **different members** have an open report
against the same visible post, the board holds that post for approval
on the spot, inside the same action that files the *N*th report. It
lands in the [approval queue](#the-approval-queue) like any held
content. A report on the opening post of a thread holds the whole
thread, every post that was showing in it; a report on a reply holds
just that reply. Approving it from the queue restores exactly what the
flag held, and rejecting it removes exactly that, with the post and
thread counts kept honest either way. A reply that was already waiting
on its own approval is left where it was. The threshold is `0` by
default, which is off; the anti-spam guide covers the reasoning.

The count is of **distinct members**, so one member filing report after
report never trips it. The reports themselves stay open, because
flagging hides the post but does not answer the reports, so you still
close them the usual way. Only reports filed **since the last hold**
count towards holding a post again, so once you approve a flag-held post
it takes a fresh set of distinct members to hold it a second time.

## Tidying threads

The **Moderator tools** panel on a thread carries the tools you hold in
that forum. It sits folded under its heading; press the heading to open
it, and it opens itself when a tool has something to tell you. Each tool
is one press, each is logged, and none of them destroys anything.
...
act on is refused and counted in the notice.

A deleted or still-unapproved thread stays in its forum's listing for
staff who may see it, with a **Deleted** or **Pending** badge and a
faint tint on the row. An ordinary member never sees the row at all.
The mark is how you find the thread to restore or approve it without
opening every one to check.
...
A post's author and staff who may edit other members' posts can open
**History** from the post actions. The history compares adjacent
revisions and records the editor, date and reason. Staff may restore an
earlier version after confirming the action; restoration runs through
the normal edit path and appends a new revision rather than deleting or
...
beside its author (a member's profile has the same), leading to the
warning screen with that post cited.

Issuing one:

- Pick a **reason** from the board's preset list, each with a title, a
  points value, and possibly an expiry, or choose **Something else**
  and set the title and points yourself (1 to 100).
- Write **what this is for**. It is required, kept on the record, and
  **shown to the member**, so write it to be read by them.
...
only when an administrator lifts it.
...
A ban is a record with a reason for the staff, a public reason the
...
- **The spam controls**: the first-post hold threshold, the hourly
  ...
  that is a settings conversation. Point the administrator at
  [Spam controls and rate limits](./antispam.md).
...
or the administrator with the thread or member in question.
...
- **[The word filter](./antispam.md#the-word-filter)** rewrites listed
  words as a page renders (post bodies, excerpts, thread titles, feeds
  and search results) without changing what is stored. You see the
  original in the queue and on the report screens, because you are
...
- **Reputation** lets members rate each other's posts: a thanks button
  ...
  warning, or a report. What the feature is on your board is a setting
