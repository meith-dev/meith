...
export async function boardEject(args: readonly string[]): Promise<number> {
  ...
  for (const [relative, contents] of files) {
    ...
    try {
      ...
      translateWriteError(error, {
        ...
        path,
        target,
        reference: 'docs/operations/custom-board.md, "Moving to a custom board"',
      })
    }
  }
  ...
  console.log('')
  console.log(
    'See docs/operations/custom-board.md, "Moving to a custom board", for the full walkthrough.',
  )
