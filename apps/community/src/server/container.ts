 ...
 *
 * It lives in `apps/community/src/server` rather than a package because assembling
...
export interface Container {
  ...
  /** F44. Null in fixture mode: drafts must not pretend to survive a restart. */
...
function buildPostgres(onBypass: (e: BypassEvent) => void): Container {
  ...
  return {
    ...
    scheduler: buildSchedulerBundle({
       ...
       * `community.config.ts` is read by the app tier and neither the worker nor
