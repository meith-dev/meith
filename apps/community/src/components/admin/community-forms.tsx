...
import {
  ...
  copyCommunityPermissionsAction,
  createCommunityAction,
  moveCommunityAction,
  removeModeratorAction,
  saveCommunityOptionsAction,
  saveCommunityPermissionsAction,
} from "@/server/community-admin-actions"
...
export interface CommunityOptionsValues {
  readonly id: number
...
export function CommunityOptionsForm({ community }: { community: CommunityOptionsValues }) {
  const [state, action] = useActionState(saveCommunityOptionsAction, EMPTY_STATE)
  ...
  return (
    <form action={action} className="flex flex-col gap-4" noValidate>
      ...
      <label className="flex flex-col gap-1 text-sm">
        ...
        <input name="title" defaultValue={community.title} className={INPUT} required />
      </label>

      <label className="flex flex-col gap-1 text-sm">
        <span className="font-medium">Slug</span>
        <input name="slug" defaultValue={community.slug} className={INPUT} required />
        <span className="text-xs text-muted-foreground">
          Appears in every link to this community. Lower-case letters, numbers and
      ...
      <label className="flex flex-col gap-1 text-sm">
        ...
        <input name="linkUrl" defaultValue={community.linkUrl} className={INPUT} />
        <span className="text-xs text-muted-foreground">
      ...
      <label className="flex flex-col gap-1 text-sm">
        ...
        <input
          ...
          name="displayOrder"
          min={0}
          defaultValue={community.displayOrder}
          className={INPUT}
        />
      </label>
        ...
        {TOGGLES.map((toggle) => (
          <label key={toggle.name} className="flex items-center gap-2 text-sm">
            <input
              ...
              name={toggle.name}
              value="1"
              defaultChecked={community.flags[toggle.name] === true}
              className="size-4"
            />
...
function effectiveLabel(cell: MatrixCell, communityTitles: ReadonlyMap<number, string>): string {
  ...
  return `inherited from ${communityTitles.get(cell.inheritedFrom) ?? "an ancestor"}: ${value}`
}
...
export function CommunityPermissionRowForm({
  communityId,
  row,
  communityTitles,
}: {
  communityId: number
  row: MatrixRow
  communityTitles: ReadonlyMap<number, string>
}) {
  const [state, action] = useActionState(saveCommunityPermissionsAction, EMPTY_STATE)
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {row.cells.map((cell) => (
          <label key={cell.key} className="flex flex-col gap-1 text-sm">
            ...
            <span className="text-xs text-muted-foreground">
              {effectiveLabel(cell, communityTitles)}
            </span>
          </label>
        ))}
      </div>
...
export function CopyPermissionsForm({
  ...
  const [state, action] = useActionState(copyCommunityPermissionsAction, EMPTY_STATE)
    ...
    <form action={action} className="flex flex-col gap-3">
      ...
      {state.notice === "copied" && (
        <p className="rounded-md border border-border bg-muted px-3 py-2 text-sm">
          Copied. Every community beneath this one now has the same permissions.
        </p>
      )}
      <input type="hidden" name="communityId" value={communityId} />
      <div>
        <SubmitButton>
          Copy to {communityCount} community{communityCount === 1 ? "" : "s"} ({changeCount} change
          {changeCount === 1 ? "" : "s"})
        </SubmitButton>
      </div>
      <p className="text-xs text-muted-foreground">
        You will be asked for your password again. This rewrites communities you are
...
export function CreateCommunityForm({
  ...
  const [state, action] = useActionState(createCommunityAction, EMPTY_STATE)
    ...
    <form action={action} className="flex flex-col gap-3" noValidate>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="flex flex-col gap-1 text-sm">
          ...
          <select name="type" defaultValue="community" className={INPUT}>
            <option value="community">Community — holds threads</option>
            <option value="category">Category — a heading that holds communities</option>
...
export function MoveCommunityForm({
  communityId,
  currentParentId,
  parents,
}: {
  ...
  const [state, action] = useActionState(moveCommunityAction, EMPTY_STATE)
  ...
  return (
    <form action={action} className="flex flex-col gap-3" noValidate>
      ...
      {state.notice === "moved" && (
        <p className="rounded-md border border-border bg-muted px-3 py-2 text-sm">
          Moved. Its subcommunities came with it.
        </p>
      )}
      <input type="hidden" name="communityId" value={communityId} />
      ...
      <div>
        <SubmitButton>Move community</SubmitButton>
      </div>
      <p className="text-xs text-muted-foreground">
        Everything beneath this community moves with it, and inherits from wherever
        it lands — so moving a busy community under a private category hides its
...
export function ModeratorsPanel({
  ...
  return (
    <section className="flex flex-col gap-4">
      ...
      {moderators.length === 0 ? (
        <p className="text-sm text-muted-foreground">Nobody moderates this community.</p>
      ) : (
                ...
                <span className="text-xs text-muted-foreground">
                  {rights.filter((right) => moderator.rights[right] === true).length === 0
                    ...
                    : rights
                        ...
                        .join(", ")}
                  {moderator.cascadeToSubcommunities && " · also in subcommunities"}
                </span>
              </span>
      ...
      <form action={appoint} className="flex flex-col gap-3 rounded-lg border border-border p-4">
        ...
        <label className="flex items-center gap-2 text-sm">
          <input type="checkbox" name="cascadeToSubcommunities" value="1" className="size-4" />
          <span>Also in every community beneath this one</span>
        </label>
