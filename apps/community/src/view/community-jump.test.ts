...
import { buildCommunityJumpModel, parseJumpTarget, type CommunityJumpRow } from './community-jump'

/**
 * F27 — the jump box's view model.
 *
 * Most of this file is about the **leak**, because that is the interesting risk
 * in a control that lists every community on the board and appears on every page.
...
const row = (
  ...
  title: string,
  type = 'community',
): CommunityJumpRow => ({ id, parentId, displayOrder: id, type, title, slug: `f${id}` })
...
const build = (visible: ReadonlySet<number>, currentCommunityId: number | null = null) =>
  buildCommunityJumpModel({ rows: TREE, visibleCommunityIds: visible, currentCommunityId })

describe('what it lists', () => {
  it('lists every visible community in tree order', () => {
    expect(build(all).communities.map((f) => f.label)).toEqual([
      'General',
      'Announcements',
      'Chat',
      'Off topic',
  ...
  it('reports depth so the theme can indent', () => {
    expect(build(all).communities.map((f) => f.depth)).toEqual([0, 1, 1, 2, 0, 1])
  })

  /*
   * Categories are listed and marked, not dropped. Dropping them would leave
   * the indentation meaningless — a community two levels deep with no visible
   * ancestor reads as top-level — and MyBB lists them too.
   */
  it('marks categories rather than omitting them', () => {
    const categories = build(all).communities.filter((f) => f.isCategory)
    expect(categories.map((f) => f.label)).toEqual(['General', 'Staff'])
  })

  it('pre-selects the community being viewed', () => {
    const selected = build(all, 3).communities.filter((f) => f.isSelected)
    expect(selected.map((f) => f.label)).toEqual(['Chat'])
  })

  it('selects nothing off a community page', () => {
    expect(build(all, null).communities.some((f) => f.isSelected)).toBe(false)
  })
})
...
describe('what it must never leak', () => {
  ...
  it('drops a whole subtree when its category is hidden', () => {
    ...
    expect(model.communities.map((f) => f.label)).toEqual([
      'General',
      'Announcements',
      'Chat',
      'Off topic',
    ])
    expect(model.communities.map((f) => f.label)).not.toContain('Staff room')
  })
  ...
  it('drops a grandchild whose parent was dropped as an orphan', () => {
    ...
    expect(model.communities.map((f) => f.label)).toEqual(['General', 'Announcements'])
    expect(model.communities.map((f) => f.label)).not.toContain('Off topic')
  })

  it('lists nothing at all when nothing is visible', () => {
    expect(build(new Set()).communities).toEqual([])
  })
})

describe('the contract with the theme', () => {
  /*
   * The app owns the URL and the parameter name. A theme typing `name="community"`
  ...
  it('supplies the action and the field name', () => {
    ...
    expect(model.field).toBe('community')
  })
  ...
  it('sends opaque string values, not hrefs the theme would have to parse', () => {
    for (const community of build(all).communities) {
      expect(community.value).toMatch(/^\d+$/)
    }
  })
})
...
describe('parseJumpTarget', () => {
  ...
  /*
   * `?community=3&community=9` reaches a page as an array where it reached the old
  ...
  /*
   * Zero is not a community id, but it is digits — so it parses, and the
