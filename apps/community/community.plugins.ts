 ...
 *
 * Split out of `community.config.ts` so that a program which needs to know *which
 * plugins this board has* does not have to import the themes as well. That is
 * not tidiness: `community.config.ts` pulls in every registered theme's slot map,
 * which is React components, and the two programs that need this list —
 * `community upgrade` and, through it, the migration planner — are plain Node
 ...
 * `community upgrade`"*, the plugin's own page said *"Run `community upgrade` before
 ...
 * installed from npm has no path to `community.config.ts`".
 *
 ...
 * gets its hooks called, its migrations applied by `community upgrade`, its settings
...
/**
 * The definitions `community upgrade` plans over.
 *
