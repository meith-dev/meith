...
export default async function AdminContentPage() {
    ...
    return (
      <PanelPage title="Content">
        <p className="mt-2 text-sm text-muted-foreground">
          This board is running on in-memory sample data, so its content settings are not
          stored.
        </p>
      </PanelPage>
    )
  ...
  return (
    <PanelPage
      ...
      lede={
        <>
          The board-wide vocabularies: what a post may contain, what gets filtered out of
          it, and what members can label a thread with.
        </>
      }
      gap="loose"
    >
      <section className="flex flex-col gap-3 rounded-lg border border-border p-4">
        ...
        <p className="text-xs text-muted-foreground">
          ...
          — because this runs on every post the board renders.{' '}
          <strong>Whole words</strong> is on by default: a filter on a fragment silently
          mangles place names and surnames, and the member it happens to has no idea why
          their post looks wrong.
        </p>

        {filters.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            No filters. Posts show as written.
          </p>
        ) : (
      ...
      <section className="flex flex-col gap-3 rounded-lg border border-border p-4">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          prefix you mistyped is not permanent.
        </p>
      ...
      <section className="flex flex-col gap-3 rounded-lg border border-border p-4">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          <code className="text-xs">:-)</code> beats <code className="text-xs">:)</code>{' '}
          at the same position. Smilies are never expanded inside a{' '}
          <code className="text-xs">[code]</code> block.
        </p>
        <p className="text-xs text-muted-foreground">
          Unlike a word filter, this changes what a post <em>renders to</em>, so editing
          it marks every stored render on the board out of date. They are rewritten in the
          background and read correctly in the meantime — nothing breaks, and a busy board
          simply does more rendering for a while. Removing a smiley puts the code back as
          the characters its author typed.
        </p>

        {smilies.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            None configured, so codes like <code className="text-xs">:)</code> show as
            typed.
          </p>
        ) : (
          <div className="flex flex-col divide-y divide-border">
      ...
      <section className="flex flex-col gap-3 rounded-lg border border-border p-4">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          That is the whole form, and the omissions are the point: there is no replacement
          template, no pattern and no HTML field, because a box that chose output markup
          would be a second markup language administered through a web form — which is how
          boards with &ldquo;custom MyCode&rdquo; acquire a permanent security hole.
        </p>
        <p className="text-xs text-muted-foreground">
          ...
          <code className="text-xs">div</code> or <code className="text-xs">span</code>{' '}
          carrying a class your theme can style. Removing one makes posts using it show
          the markup as written, which is what an undefined directive has always done.
        </p>
