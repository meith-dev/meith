...
describe('inlineModerateAction', () => {
  it('applies the tool and reports the outcome in the query string', async () => {
    const where = await redirectOf(
      inlineModerateAction(
        EMPTY_STATE,
        form({ tool: 'lock', returnTo: '/forum/2-general' }, ['thread:20', 'thread:21']),
      ),
    )

    expect(where).toBe('/forum/2-general?did=lock&n=2')
  ...
  it('names every count that is not zero', async () => {
    ...
    const where = await redirectOf(
      inlineModerateAction(
        EMPTY_STATE,
        form({ tool: 'lock', returnTo: '/forum/2-general' }, [
          'thread:20',
          'thread:21',
          'thread:99',
        ]),
    ...
    expect(where).toBe('/forum/2-general?did=lock&n=1&gone=1&skipped=1')
  })
  ...
  it('drops a malformed selection value rather than guessing at it', async () => {
    ...
    expect(where).toBe('/forum/2-general?did=lock&n=1')
  ...
  it('reports a thread outside the scope exactly as it reports one that does not exist', async () => {
    ...
    const elsewhere = await redirectOf(
      inlineModerateAction(
        EMPTY_STATE,
        form({ tool: 'lock', returnTo: '/forum/2-general' }, ['thread:20']),
      ),
    )
    ...
    expect(elsewhere).toBe('/forum/2-general?did=lock&n=0&gone=1')
  })
    ...
    it('appends to a path that already carries a query', async () => {
      expect(
        await redirectOf(
          inlineModerateAction(
            EMPTY_STATE,
            form({ tool: 'lock', returnTo: '/forum/2-general?page=2' }, ['thread:20']),
          ),
        ),
      ).toBe('/forum/2-general?page=2&did=lock&n=1')
    })
  })
