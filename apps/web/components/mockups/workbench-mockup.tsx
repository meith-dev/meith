...
const tabs = [
  { icon: TerminalSquare, label: "Agent", active: true },
  { icon: FileText, label: "report.md", active: false },
  { icon: Globe, label: "localhost:3000", active: false },
]

/**
 * Faithful static recreation of the meith desktop workbench, built from the
 * real renderer surfaces: the far-left Spaces rail, a tab strip, the agent
 * transcript (with a tool-call card and a permission prompt), a side preview
 * pane, and the runtime status bar. Decorative only — `aria-hidden`.
 */
    ...
    >
      ...
      <div className="flex h-9 items-center gap-2 border-b border-border bg-card px-3">
        ...
        <div className="mx-auto flex items-center gap-1.5 text-xs text-muted-foreground">
          <MeithMark className="size-3.5 text-foreground" />
      ...
      <div className="flex h-[420px]">
        {/* Spaces rail */}
        <nav className="flex w-14 shrink-0 flex-col items-center border-r border-border bg-card/60 py-3">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          {/* Tab strip */}
          <div className="flex h-9 items-center gap-1 border-b border-border bg-card/40 px-2">
            {tabs.map((t) => (
              <div
                key={t.label}
                className={cn(
            ...
            <div className="flex min-w-0 flex-1 flex-col">
              <div className="flex-1 space-y-3 overflow-hidden p-3.5 text-[13px] leading-relaxed">
                ...
                <div className="max-w-[88%] text-foreground/90">
                  On it. I&apos;ll read the report first, then bring up the
                  preview.
                </div>
