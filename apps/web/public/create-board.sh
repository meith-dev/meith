...
FROM ghcr.io/meith-dev/meith-base:0.17.0 AS deps
WORKDIR /board
...
mkdir -p "$(dirname -- "$BOARD_NAME/compose.yml")"
cat > "$BOARD_NAME/compose.yml" <<'MEITH_SCAFFOLD_EOF'
...
**Building it yourself**: `docker build -t __MEITH_BOARD_NAME__ .` works on any machine
...
echo "Created $BOARD_NAME — 13 files."
echo
echo "  cd $BOARD_NAME"
echo "  npm install"
