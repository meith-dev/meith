/**
 * F21: "`visibleCommunityIds` is one query."
 *
...
describe('visibleCommunityIds', () => {
  /*
   * Was 32 queries on this 15-community board — two per community plus overhead. Every
   ...
   */
  it('costs a constant number of queries, not one per community', async () => {
   ...
   * on a three-community fixture with a per-community walk; only comparing two board
   * sizes proves the cost does not grow.
   */
  it('costs the same on a board five times the size', async () => {
    const bigger = await createTestDb()
    try {
      await seedBoard(bigger.db, { ...SMOKE_SCALE, communities: 60, categories: 5, threads: 5 })
      ...
      const small = await measureQueries(harness, () => authorizer.visibleCommunityIds(actor!))
      const large = await measureQueries(bigger, () =>
        bigAuthorizer.visibleCommunityIds(bigActor),
      )
