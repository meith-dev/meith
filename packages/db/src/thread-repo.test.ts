...
import { communities, threads } from './schema'
...
describe('PostgresThreadRepository.listCommunity', () => {
  it('orders sticky threads first and resumes with the full stable sort key', async () => {
    await seed(5)

    const first = await repo.listCommunity(1, { limit: 2, scope: PUBLIC_CONTENT })
    const second = await repo.listCommunity(1, {
  ...
  it('costs one statement at both small and larger board sizes', async () => {
    ...
    await expectQueryBudget(harness, 1, () =>
      repo.listCommunity(1, { limit: 25, scope: PUBLIC_CONTENT }),
    )
  ...
  it('sorts rating aggregates with a stable next-page cursor', async () => {
    ...
    const first = await repo.listCommunity(1, {
      limit: 2,
      scope: PUBLIC_CONTENT,
      sort: 'rating',
    })
