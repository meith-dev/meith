 ...
 *    a single fixture cannot tell "one query" from "one query per community" when
 *    the fixture has one community.
 *  - **the null shapes.** A community with no posts has every last-post column null,
 *    and a community whose author was deleted has `last_post_user_id` null with the
...
/** `count` sibling communities under one category, each with a last post. */
...
describe('listListing', () => {
  it('returns counters and the last post for every community', async () => {
    await seedBoard(2)
  ...
  it('reports no last post for a community that has none', async () => {
    await seedBoard(0)
  ...
  it('keeps the last post when its author was deleted', async () => {
    await seedBoard(1)
    await db.execute(sql`update communities set last_post_user_id = null where id = 100`)
  ...
  it('treats a partially-written last post as absent', async () => {
    await seedBoard(1)
    await db.execute(sql`update communities set last_post_id = null where id = 100`)
  ...
  it('orders by display order, then id', async () => {
    await seedBoard(3)
    await db.execute(sql`update communities set display_order = 99 where id = 100`)
   ...
   * query" and "one query per community" are the same number. Twenty communities against
   * three must still be one statement.
   */
  it('costs one query regardless of how many communities exist', async () => {
    await seedBoard(2)
  ...
  it('costs one query regardless of depth', async () => {
    await db.insert(communities).values([
      ...
      { id: 2, type: 'community', title: 'A', slug: 'a', parentId: 1, path: '1.2', depth: 1, displayOrder: 0 },
      { id: 3, type: 'community', title: 'B', slug: 'b', parentId: 2, path: '1.2.3', depth: 2, displayOrder: 0 },
      { id: 4, type: 'community', title: 'D', slug: 'd', parentId: 3, path: '1.2.3.4', depth: 3, displayOrder: 0 },
    ])
