 ...
 *
 * `community_permissions` has had a reader since F21 and no writer at all, so every
...
import { MODERATOR_RIGHTS } from './community-admin-repo'

import type { Database } from './client'
import { createTestDb, type TestDb } from './pglite.fixture'
import { PostgresCommunityAdminRepository } from './community-admin-repo'
import { resultRows } from './result-rows'

let harness: TestDb
let db: Database
let repo: PostgresCommunityAdminRepository
...
beforeAll(async () => {
  ...
  db = harness.db
  repo = new PostgresCommunityAdminRepository(db)
}, 60_000)
...
beforeEach(async () => {
  await db.execute(sql`delete from community_permissions`)
  await db.execute(sql`delete from communities`)
  await db.execute(sql`
    insert into communities (id, type, title, slug, path, depth, display_order)
    values (${ROOT}, 'category', 'Root', 'root', '10', 0, 1),
           (${CHILD}, 'community', 'Child', 'child', '10.20', 1, 1),
           (${GRANDCHILD}, 'community', 'Grandchild', 'grandchild', '10.20.30', 2, 1),
           (${DECOY}, 'community', 'Decoy', 'decoy', '10.200', 1, 2)
  `)
})

/** Every community-scoped key, all null but the ones given. */
function values(over: Record<string, boolean | number | null> = {}) {
  const out: Record<string, boolean | number | null> = {}
  for (const field of COMMUNITY_PERMISSION_FIELDS) out[field.key] = null
  return { ...out, ...over }
}

async function storedRowCount(): Promise<number> {
  const rows = resultRows(
    await db.execute(sql`select count(*)::int as n from community_permissions`),
  ) as Array<{ n: number }>
    ...
    const [row] = resultRows(
      await db.execute(sql`
        select title, slug, description, display_order, is_open, allow_threads,
          ...
          from communities where id = ${CHILD}
      `),
...
describe('overrides', () => {
  ...
  it('leaves a null out of what it reads back, so the walk does not stop on it', async () => {
    /*
     * `CommunityOverride.overrides` holds only the keys that are set. A
     * present-but-null key would stop the resolver's ancestor walk at the wrong
     * community — which is the bug that makes a parent's change do nothing.
     */
    await repo.saveOverrides(CHILD, REGISTERED, values({ canPostThreads: false }))
  ...
  it('deletes a row whose every cell is inherit', async () => {
     ...
     * permission check on every page — and an operator who cleared a community's
  ...
  it('reads several communities at once, which the matrix needs for the chain', async () => {
  ...
  it('is empty for no communities, without asking the database', async () => {
    expect(await repo.readOverrides([])).toEqual([])
  })

  it('goes with the community, by cascade', async () => {
    await repo.saveOverrides(CHILD, REGISTERED, values({ canPostThreads: false }))
    await db.execute(sql`delete from communities where id = ${CHILD}`)
...
describe('descendantIds', () => {
  ...
  it('does not match a sibling whose path merely starts the same', async () => {
     ...
     * one community would reach into another subtree entirely — silently, and with
...
describe('copyToDescendants', () => {
  it('makes a descendant identical, including clearing what it had', async () => {
     ...
     * operator would be looking at two communities they were told are the same.
  ...
  it('touches only the communities it was given', async () => {
    await repo.saveOverrides(CHILD, REGISTERED, values({ canPostThreads: false }))
...
describe('moderator appointments', () => {
  ...
  it('replaces the rights of an existing appointment rather than duplicating it', async () => {
    ...
    expect(rows[0]?.cascadeToSubcommunities).toBe(true)
  ...
  it('refuses an appointment that names both a member and a group, or neither', async () => {
     ...
     */
    const base = { communityId: CHILD, cascadeToSubcommunities: false, rights: noRights }
  ...
  it('keeps two people apart on the same community', async () => {
    const base = { communityId: CHILD, groupId: null, cascadeToSubcommunities: false, rights: noRights }
  ...
  it('removes one appointment, and only from the community it belongs to', async () => {
    /*
     * The id comes from a form and is therefore attacker-supplied. Scoping the
     * delete to the community means an id from another community matches nothing rather
  ...
  it('goes with the community, and with the member, by cascade', async () => {
    const base = { communityId: CHILD, groupId: null, cascadeToSubcommunities: false, rights: noRights }
    await repo.appoint({ ...base, userId: 1 })
