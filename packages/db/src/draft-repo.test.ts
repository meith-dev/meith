...
beforeEach(async () => {
  ...
  await db.execute(sql`insert into communities (id, type, title, slug, path) values (900, 'community', 'Test', 'test', '900')`)
})

describe('PostgresDraftRepository', () => {
  it('keeps one new-thread draft per user and community', async () => {
    await drafts.save(901, { communityId: 900, threadId: null, title: 'First', message: 'one', prefixId: null })
    await drafts.save(901, { communityId: 900, threadId: null, title: 'Second', message: 'two', prefixId: null })
