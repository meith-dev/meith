 ...
 * `apps/community` maps nodes into a view model.
 */
...
export interface CommunityRow extends TreeShaped {
...
export type CommunityNode<T extends TreeShaped = CommunityRow> = T & {
  readonly children: readonly CommunityNode<T>[]
}

/**
 * The denormalised "last post in this community" triplet (F29).
 *
 * Carried on `communities` rather than derived, because deriving it means a
 * correlated subquery per community on the board index — the page every visitor
...
/**
 * A community row with the volatile columns a listing shows (F29).
 *
 * Kept separate from `CommunityRow` on purpose — see `CommunityRepository.listListing`
 * for why these two reads must not share a cache entry.
 */
export interface CommunityListingRow extends CommunityRow {
  readonly threadCount: number
  readonly postCount: number
  readonly lastPost: LastPostSummary | null
}

/** Everything needed to create a community. `path`/`depth` are derived, never given. */
...
/** Where a community is being moved to, and where among its new siblings. */
export interface MoveTarget {
  /** `null` moves the community to the top level. */
...
export interface MovePlan {
  ...
  readonly newParentId: number | null
  /** The moved community's own new path, and every descendant's. */
