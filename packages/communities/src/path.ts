 ...
 *
 * `communities.path` is dot-separated ancestor ids, root first, **inclusive of self**:
 *
 *     Category (id 1)              '1'
 *       Community (id 4)               '1.4'
 *         Subcommunity (id 9)          '1.4.9'
 *
 ...
 * matches `'1.40'`, a *sibling*, so a reparent would drag unrelated communities with
...
export function parsePath(path: string): number[] {
  ...
  return path.split(PATH_SEPARATOR).map((segment) => {
    ...
    if (!Number.isInteger(id) || id <= 0) {
      throw new ValidationError(`Malformed community path segment: ${segment}`)
    }
    return id
  })
}
...
/**
 * The ids of every ancestor, root first, **excluding** the community itself — the
...
/** Depth is the number of ancestors: a root community is depth 0. */
