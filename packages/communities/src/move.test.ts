/**
 * F16 acceptance: "Reparent a mid-tree community; every descendant path is correct."
 *
 ...
 *  2 Category two
 */
const TREE: CommunityRow[] = [
  community(1, null, '1', { type: 'category', slug: 'cat-one', displayOrder: 0 }),
  community(2, null, '2', { type: 'category', slug: 'cat-two', displayOrder: 1 }),
  community(4, 1, '1.4', { slug: 'general' }),
  community(9, 4, '1.4.9', { slug: 'off-topic' }),
  community(12, 9, '1.4.9.12', { slug: 'deep' }),
]
...
describe('planMove — rejections', () => {
  it('refuses to move a community into its own descendant', () => {
  ...
  it('does not confuse a sibling whose id shares a prefix', () => {
    const rows = [...TREE, community(40, 1, '1.40', { slug: 'forty' })]
...
describe('planMove — sibling ordering', () => {
  it('appends when no position is given', () => {
    ...
    // Roots are 1 and 2, so the appended community lands third.
...
describe('applying a plan keeps the tree walkable', () => {
  it('rebuilds correctly from the rewritten rows', () => {
    ...
    // Every community still reachable from a root, none orphaned or duplicated.
