...
describe('who may change or remove an event', () => {
  it('lets the member who added it remove it', () => {
    expect(mayManage({ userId: 7, createdByUserId: 7, organisers: [] })).toBe(true)
  })

  it('lets an organiser remove somebody else’s', () => {
    expect(mayManage({ userId: 9, createdByUserId: 7, organisers: [9] })).toBe(true)
  })

  it('refuses an unrelated member, even where any member may add', () => {
    expect(mayManage({ userId: 8, createdByUserId: 7, organisers: [] })).toBe(false)
  })

  it('refuses a guest', () => {
    expect(mayManage({ userId: null, createdByUserId: null, organisers: [] })).toBe(false)
  })

  it('does not let a guest-created event be removed by any signed-in member', () => {
    expect(mayManage({ userId: 8, createdByUserId: null, organisers: [] })).toBe(false)
  })
})
